...
text_reviews <- clean_pizza_hut_reviews %>%
  filter(text_indicator == 1)

text_reviews |>
  ggplot(aes(x = factor(stars), y = words)) +
  geom_boxplot() +
  scale_y_log10() +  # This adds the log transformation to the y-axis
  theme_classic() +
  labs(
    x = "Stars Rating",
    y = "Log of Word Count"
...
modelsummary(
  list(
    "Give a Text Review" = text_stars_model
  ),
  statistic = "mad",
  booktabs = TRUE
)
...
modelplot(text_stars_model, conf_level = 0.9) +
...
modelsummary(
  list(
    "Neg binomial" = words_stan_glm,
    "Multilevel Neg Binomial" = words_stan_glmer
  )
)
